...
func NewCollection() *Collection {
	...
	c.table = make([]*entry, defaultInitialCapacity)
...
//GetFirst
func (c Collection) GetFirst() (*engine.ObjectIndex, bool) {
	//c.mutex.RLock()
	//defer //c.mutex.RUnlock()
	var i uint32 = 0
	t := c.table[i]
...
//GetLast
func (c Collection) GetLast() (*engine.ObjectIndex, bool) {
	//c.mutex.RLock()
	//defer //c.mutex.RUnlock()
	var i uint32 = 1
...
// specified key.
func (c Collection) ContainsKey(key *engine.ObjectId) bool {
...
func (c *Collection) transfer(newTable []*entry) {
	src := c.table
...
func (c Collection) Remove(key *engine.ObjectId) (*engine.ObjectIndex, bool) {
...
func (c Collection) Iter() <-chan interface{} {
	v := make(chan interface{}, int(math.Sqrt(float64(c.size))))
